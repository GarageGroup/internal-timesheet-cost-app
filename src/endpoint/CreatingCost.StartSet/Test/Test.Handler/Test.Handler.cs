...
partial class CreatingCostStartHandlerTest
{
    ...
    {
        var mockOrchestration = BuildMockOrchestration<CreatingCostSetOrchestrateIn>(SomeOrchestrationOut);
        var handler = new CreatingCostSetStartHandler(mockOrchestration.Object);
        ...
        mockOrchestration.Verify(f => f.ScheduleInstanceAsync(expectedInput, cancellationToken), Times.Once);
    }

    [Theory]
    [InlineData(HandlerFailureCode.Transient)]
    [InlineData(HandlerFailureCode.Persistent)]
    public static async Task HandleAsync_OrchestrationResultIsFailure_ExpectFailure(HandlerFailureCode sourceFailureCode)
    {
        var sourceException = new Exception("Some exception message");
        var orchestrationFailure = sourceException.ToFailure(sourceFailureCode, "Some failure text");

        var mockOrchestration = BuildMockOrchestration<CreatingCostSetOrchestrateIn>(orchestrationFailure);
        var handler = new CreatingCostSetStartHandler(mockOrchestration.Object);
    ...
    {
        var orchestrationInstanceScheduleOut = new OrchestrationInstanceScheduleOut(
            instanceId: BuildOrchestrationInstanceId("InstanceId"));
        var mockOrchestration = BuildMockOrchestration<CreatingCostSetOrchestrateIn>(orchestrationInstanceScheduleOut);

        var handler = new CreatingCostSetStartHandler(mockOrchestration.Object);
        var cancellationToken = new CancellationToken(canceled: false);

        var actual = await handler.HandleAsync(SomeInput, cancellationToken);
        var expected = BuildOrchestrationInstanceId("InstanceId");

        Assert.Equal(expected.Id, actual.SuccessOrThrow().Id);
    }
}